...
def handler(event, context):
    ...
    try:
        ...
        sv(DB, 'C3', proj); sv(DB, 'C4', S.get('site', ''))
        sv(DB, 'C5', S.get('client', '')); sv(DB, 'C8', S.get('operator', ''))
        if notes: sv(DB, 'C24', notes)

        sv(TO, 'C2', S.get('client', '')); sv(TO, 'H2', proj)
        sv(TO, 'C3', S.get('site', '')); sv(TO, 'H3', S.get('operator', ''))
        sv(TO, 'H4', quote_date); sv(TO, 'H5', quote_num)
        ...
        if fv(irrig.get('matCost')):
            ...
            sv(TO, 'E25', 1)
            sv(TO, 'F25', fv(irrig.get('matCost')))
        ...
        if fv(mob.get('fuelLoads')):
            sv(TO, 'E57', fv(mob.get('fuelLoads')))
            sv(TO, 'F57', fv(mob.get('fuelRate', 45)))
        if fv(mob.get('dumpQty')):
            sv(TO, 'E58', fv(mob.get('dumpQty')))
            sv(TO, 'F58', fv(mob.get('dumpRate', 80)))
        if fv(mob.get('delQty')):
            sv(TO, 'E59', fv(mob.get('delQty')))
            sv(TO, 'F59', fv(mob.get('delRate', 120)))
        ...
        return {'statusCode': 200, 'headers': headers, 'body': json.dumps({'file': b64})}

    except Exception as e:
        tb = traceback.format_exc()
        print(f"ERROR: {e}\n{tb}", file=sys.stderr)
        return {'statusCode': 500, 'headers': headers,
                'body': json.dumps({'error': str(e), 'trace': tb[-800:]})}
